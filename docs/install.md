...
2. Create a new directory for your nix-bitcoin deployment and copy initial files from nix-bitcoin
   ```
   $ mkdir nix-bitcoin-node
   $ cd nix-bitcoin-node
   $ cp -r ../nix-bitcoin/examples/{configuration.nix,nixops,shell.nix} .
   ```
